...
import { 
  getEffectiveVehiclePhotos, 
  getEffectiveGoalPhotos, 
  autoSyncLocalPhotosToServer, 
  syncVehiclePhotosToServer 
} from '../lib/vehiclePhotosService';
...
// 5 Metas de Infraestructura a Largo Plazo ($28,000)
...
export const FinanciaModal: React.FC<FinanciaModalProps> = ({
  ...
  // Estado para fotos personalizadas de vehículos (cargadas desde el bundle del servidor y localStorage)
  const [customVehiclePhotos, setCustomVehiclePhotos] = useState<Record<string, string>>(() => {
    return getEffectiveVehiclePhotos();
  });
  ...
  const [customGoalPhotos, setCustomGoalPhotos] = useState<Record<string, string>>(() => {
    return getEffectiveGoalPhotos();
  });

  // Auto-sincronización inicial: Si el usuario abre la web desde su PC con fotos guardadas en localStorage,
  // se envían inmediatamente al servidor para que el teléfono móvil y los donantes las vean automáticamente.
  useEffect(() => {
    let isMounted = true;
    autoSyncLocalPhotosToServer().then(synced => {
      if (synced && isMounted) {
        setSyncStatus('Fotos sincronizadas con el servidor');
        setTimeout(() => {
    ...
    // Consultar el servidor para refrescar si otro dispositivo o sesión guardó fotos
    fetch('/api/sync-vehicle-photos')
      .then(res => res.json())
      .then(data => {
        if (!isMounted) return;
        if (data && (data.vehiclePhotos || data.goalPhotos)) {
          if (data.vehiclePhotos) {
            setCustomVehiclePhotos(prev => ({ ...data.vehiclePhotos, ...prev }));
          }
          if (data.goalPhotos) {
            setCustomGoalPhotos(prev => ({ ...data.goalPhotos, ...prev }));
          }
        }
      })
      .catch(() => {});
  ...
  const handleManualSyncToServer = async () => {
    try {
      setIsSyncing(true);
      setSyncStatus('Sincronizando fotos con servidor...');
      const res = await syncVehiclePhotosToServer(customVehiclePhotos, customGoalPhotos);
      if (res.success) {
        setSyncStatus('¡Fotos fijadas como definitivas en servidor para móviles y web!');
        setTimeout(() => setSyncStatus(null), 5000);
      } else {
        setSyncStatus(`Error: ${res.error || 'No se pudo sincronizar'}`);
      }
    } catch (err: any) {
    ...
    reader.onload = async (e) => {
      ...
      if (dataUrl) {
        setCustomVehiclePhotos(prev => {
          const next = { ...prev, [vehicleId]: dataUrl };
          return next;
        });
        try {
        ...
        // Guardar instantáneamente en servidor
        setSyncStatus('Guardando en servidor...');
        const res = await syncVehiclePhotosToServer({ [vehicleId]: dataUrl }, {});
        if (res.success) {
          setSyncStatus('✓ Foto definitiva guardada para móviles y web');
  ...
  const handleResetVehiclePhoto = async (vehicleId: string) => {
    setCustomVehiclePhotos(prev => {
      const next = { ...prev };
    ...
    await syncVehiclePhotosToServer({ [vehicleId]: '' }, {});
  };
    ...
    reader.onload = async (e) => {
      ...
      if (dataUrl) {
        setCustomGoalPhotos(prev => {
          const next = { ...prev, [goalId]: dataUrl };
          return next;
        });
        try {
        ...
        // Guardar instantáneamente en servidor
        setSyncStatus('Guardando en servidor...');
        const res = await syncVehiclePhotosToServer({}, { [goalId]: dataUrl });
        if (res.success) {
          setSyncStatus('✓ Foto definitiva guardada para móviles y web');
          setTimeout(() => setSyncStatus(null), 4000);
        }
  ...
  const handleResetGoalPhoto = async (goalId: string) => {
    setCustomGoalPhotos(prev => {
      const next = { ...prev };
      delete next[goalId];
      return next;
    ...
    await syncVehiclePhotosToServer({}, { [goalId]: '' });
  };
  ...
  const longTermGoals: LongTermGoal[] = [
    ...
    },
    {
      ...
      badgeColor: '',
      icon: Bike,
      image: '/vehicles/moto-carga-2026.jpg',
    ...
    },
    {
      ...
      icon: Shirt,
      image: '/vehicles/uniformes-brigada.jpg',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 overflow-y-auto bg-black/85 backdrop-blur-md animate-fade-in">
      ...
      >
        ...
        <div className="relative z-10 overflow-y-auto px-5 sm:px-7 py-5 flex-1">
          {/* Caja Interior FASE AMPLIA con Barra de Progreso Macro ($28,000 USD) */}
              ...
              <button
                ...
                onClick={() => {
                  startDonationFlow({
                    ...
                    suggestedAmount: 5,
                    isMacro: false // Donación directa / microdonación independiente de las metas de $28.000
                  });
                }}
                className={`p-3.5 sm:p-4 rounded-2xl border text-left transition-all cursor-pointer flex items-center justify-between gap-3 ${
                ...
                {longTermGoals.map((goal, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="bg-black/50 border border-white/15 rounded-2xl p-4 mb-4">
                          {/* Barra de estado de sincronización multidispositivo */}
                          {syncStatus && (
                            <div className="mb-3 px-3 py-1.5 rounded-lg bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs flex items-center justify-between animate-fade-in">
                              <span className="flex items-center gap-1.5 font-medium">
                                <CheckCircle2 size={14} className="text-emerald-400 shrink-0" />
                                {syncStatus}
                              </span>
                              <span className="text-[10px] text-stone-400 font-mono">Multidispositivo (Web y Móvil)</span>
                            </div>
                          )}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                            ...
                            <div className="md:col-span-6">
                              <div className="relative rounded-xl overflow-hidden border border-white/20 h-64 sm:h-72 md:h-80 w-full bg-black/80 group shadow-lg">
                                ...
                                {/* Controles de edición (visibles al pasar el cursor o en Modo Edición) */}
                                {(isEditMode || !customVehiclePhotos[currentVehicle.id]) && (
                                  <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1.5">
                                    <label 
                                      className="px-2.5 py-1.5 rounded-lg bg-black/85 hover:bg-black text-white text-[11px] font-semibold border border-white/25 shadow-lg backdrop-blur-md transition-all cursor-pointer flex items-center gap-1.5 active:scale-95"
                                      title="Subir y sincronizar foto definitiva para móviles y web"
                                    >
                                      <Camera size={13} className="text-[#00f0ff]" />
                                      <span>Cambiar foto definitiva</span>
                                      <input 
                                        type="file" 
                                        accept="image/*" 
                                        className="hidden" 
                                        onChange={(e) => {
                                          ...
                                          if (file) handleUploadVehiclePhoto(currentVehicle.id, file);
                                        }} 
                                      />
                                    </label>
                                    {customVehiclePhotos[currentVehicle.id] && isEditMode && (
                                      <button
                                        type="button"
                                        onClick={() => handleResetVehiclePhoto(currentVehicle.id)}
                                        className="p-1.5 rounded-lg bg-red-950/80 hover:bg-red-900 text-red-200 border border-red-500/30 text-xs shadow transition-all cursor-pointer"
                                        title="Restaurar foto de origen"
                                      >
                                        <RotateCcw size={13} />
                                      </button>
                                    )}
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Visualización limpia de foto para Moto y Uniformes */}
                      {(goal.id === 'moto-carga' || goal.id === 'uniformes') && (
                        <div className="mb-3">
                          <div className="rounded-xl overflow-hidden border border-white/20 relative group max-h-56 bg-black/60 shadow-md">
                            <img 
                              ...
                              onError={(e) => {
                                // graceful fallback to default asset if file missing
                                (e.target as HTMLImageElement).src = goal.id === 'moto-carga' 
                                  ? '/vehicles/chevrolet-dmax.jpg' 
                                  : '/vehicles/hilux-azul-2008.jpg';
                              }}
                            />

                            {/* Control de edición solo visible en Modo Edición */}
                            {isEditMode && (
                              <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1.5">
                                <label 
                                  className="px-2.5 py-1.5 rounded-lg bg-black/85 hover:bg-black text-white text-[11px] font-semibold border border-white/25 shadow-lg backdrop-blur-md transition-all cursor-pointer flex items-center gap-1.5 active:scale-95"
                                ...
                                >
                                  <Camera size={13} className="text-[#00f0ff]" />
                                  <span>Actualizar foto definitiva</span>
                                  <input 
                                    type="file" 
                                    accept="image/*" 
                                    className="hidden" 
                                    onChange={(e) => {
                                      const file = e.target.files?.[0];
                                      if (file) handleUploadGoalPhoto(goal.id, file);
                                    }} 
                                  />
                                </label>
                                {customGoalPhotos[goal.id] && (
                                  <button
                                    type="button"
                                    onClick={() => handleResetGoalPhoto(goal.id)}
                                    className="p-1.5 rounded-lg bg-red-950/80 hover:bg-red-900 text-red-200 border border-red-500/30 text-xs shadow transition-all cursor-pointer"
              ...
              {microStep === 'monto' && (
                <div>
                  ...
                  {/* Ítems en los que se redirigen estos apoyos:
                      SOLAMENTE se muestran en donaciones libres / 1$ que NO dependan de la meta de $28.000 */}
